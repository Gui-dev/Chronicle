...
## Follow-ups (bugs pré-existentes)

- [ ] Feature de música morta no browser: `use-spotify-search` consome `SpotifyTrack` (tipo plano) vs resposta bruta do Spotify com `album.images[].url` — covers não aparecem no wizard
- [ ] `audio-player.tsx` ainda usa `<img>` (migrar para `next/image`)
- [ ] Drift em `packages/db/src/schema/auth.ts`: auth espera text IDs (varchar) mas schema declara uuid
- [ ] Scripts `db:*` do root quebrados (drizzle) em `package.json`
- [ ] `drizzle.config.ts` recursa em `__tests__` ao gerar migrações
- [ ] E2E: firefox não instalado no Playwright (rodar com `--project=chromium`)
- [ ] E2E: runs repetidos acumulam memórias duplicadas no DB dev
